...
class FlutterWaveController extends GetxController {
  ...
  void getUrl() {
    url.value = _paymentRepository.getFlutterWaveUrl(booking.value);
  }

  void showConfirmationIfSuccess() {
    final _doneUrl =
        "${Helper.toUrl(Get.find<GlobalService>().baseUrl)}payments/flutterwave";
    if (url == _doneUrl) {
      Get.find<BookingsController>().currentStatus.value =
          Get.find<BookingsController>().getStatusByOrder(50).id;
